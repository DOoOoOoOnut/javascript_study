...
## 매개변수와 return
* 함수 호출 시 전달되는 값을 받는 함수 내 생성 변수 재사용되는 함수에서 변경되는 데이터 처리가능
* `function함수명(매개변수명){return실행} 함수명(매개변수)`

### 매개변수
* 함수 내 데이터가 호출시마다 달라질 때 사용
* 호출할때마다 생성한 매개변수에 인수값이  대입됨.

### return
* 실행 결과가 매번 다른 경우 데이터 값만 함수 외부로 반환할 때 사용
* 주의사항 : 리턴을 만나면 함수가 종료된다. 리턴 아래 스크립트를 읽지 못함.
...
* 이벤트 안에서 현재 대상을 가리킴
* this.type 현 대상의 속성
* 직접 객체 변수명을 쓰는 것보다 편하고 오타 가능성이 줄어드는 장점

* 객체 내 함수 안에서도 쉽게 사용 가능
* `const ob = [{`
    `w:50,`
    `h:100,`
    `func:function(){`
...
`},{`
    ...
    `h:40,`
`}]`
* 객체 내 함수에서도 인덱스 꼭 써줘야 오류 없음~ 항상 문법에 맞게 쓰자!
...
* 주의 : 화살표 함수 내에서 this 인식 불가!

## if 조건문

### 논리데이터(Boolean)

* 참(true) 또는 거짓(false) 두 가지 값 중 하나의 데이터형식
* 조건문, 논리연산자, 논리적 판단 등에 사용
* 숫자데이터와 함께 사용할 때는 0,1로 같이 계산됨

### 비교연산자

* 두 값 이상을 비교하여 그 결과를 논리데이터로 반환하는 연산자
* 데이터타입까지 정확하게 비교하는 방법을 엄격 방식이라고 함
...
* 비교연산 또는 논리데이터 좌우 두 값으로두고 논리적으로 비교
* `AND(&&) OR(||) NOT(!)`
* 우선순위 : NOT >> AND >> OR
* AND(&&) : 양쪽 피연산자가 모두 참일 때 결과는 참
=> 예시: 아이디와 비밀번호가 일치하는가? true> 로그인 false> 로그인 실패
* OR(||) : 양쪽 피연산자 중 하나라도 참이면 결과는 참
* NOT(!) : 피연산자의 논리 값을 반전함. 참이면 거짓 반환

## 조건문 if문
* 주어진 조건에 따라 결과가 참/거짓으로 나뉠 때 참에 대한 결과를 실행
...
## RegEx 정규표현식
* 정규표현식(Regular expression)은 문자열 패턴 매칭을 위한 도구
* 특정 규칙을 따르는 문자열을 검색하거나 변형하는데 사용한다
* 예시 -> 아이디가 영어와 숫자로 이루어져 있는가 / 비밀번호에 특수문자를 포함했는가/ 이메일이 메일형식에 맞게 입력됐는가/ 이름이 한글로 입력됐는가/ 카드비밀번호를 숫자로 입력했는가
...
* `/별로/.test('구매하지 마세요. 진짜 별로에요!')`
---

## if -> 조건대상의 값, 비교 연산자, 논리연산자 등의 대상이 넓을 경우 사용
